...
pub fn read_trust_vector_from_csv(
    input: &str, peer_indices: &HashMap<String, usize>
    ...
    for line in input.lines() {
        ...
        let (peer, level) = match fields.len() {
            ...
            _ => {
                let peer = parse_peer_id(fields[0], peer_indices)
                    .map_err(|e| format!("invalid peer {:?} in line {}: {}", fields[0], count, e))?;
                let level = if fields.len() >= 2 {
                    parse_trust_level(fields[1])
                        .map_err(|e| format!("invalid trust level {:?} in line {}: {}", fields[1], count, e))?
                } else {
                    1.0
                };
                (peer, level)
        ...
        entries.push(Entry { index: peer, value: level });
    }
